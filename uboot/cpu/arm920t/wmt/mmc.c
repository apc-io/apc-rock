...
#define BA_SDC_3 			0xD800AC00	// SD Controller Base Address
#define BA_SDCDMA_3 		0xD800AD00	// SD Controller DMA Base Address
...
sd_info_t SD3DevInfo; //add by eason 2012/3/27
...
static block_dev_desc_t mmc3_dev; //add by eason 2012/3/27 

static int mmc_ready = 0;
static int mmc0_ready = 0;
static int mmc1_ready = 0;
static int mmc2_ready = 0;
static int mmc3_ready = 0; // add by eason 2012/3/27
...
void Change_SD_host(int host_num)
{
	...
	case 3:
		if (chip_id == 0x3481) {
			pSd_Reg = (PWMT_SDMMC_REG) BA_SDC_3;
			pSd_PDma_Reg = (struct _SD_PDMA_REG_ *) BA_SDCDMA_3;
			SDDevInfo = &SD3DevInfo;
			mmc_dev = &mmc3_dev;
			mmc_ready = mmc3_ready;
		}
		break;
	default:
		break;
	...
	for (i = 1; i <= 4; i++) {
		if (card_status[i] != card_status_ready[i]) {
				if(card_status[4] == 0x40) {
						printf("\n !! Warning !! pSd_Reg->Rsp[4]=%X ! \n", pSd_Reg->resr[4]) ; //back operation support
			}					
			else {
					SDPRINTK("\n pSd_Reg->Rsp[1]=%X ! \n", pSd_Reg->resr[1]) ;
					SDPRINTK("\n pSd_Reg->Rsp[2]=%X ! \n", pSd_Reg->resr[2]) ;
...
block_dev_desc_t * mmc_get_dev(int dev)
{
	if (dev == 0)
		...
		return ((block_dev_desc_t *)&mmc3_dev);
	return NULL;
}
...
int sd_set_clock(int Frequence)
{
	...
	case SD_Clk_15MHz:
		...
		if (SDDevInfo->CtrlID == 0) {
			auto_pll_divisor(DEV_SDMMC0, SET_DIV, 2, 15); //change freq*2 to freq by eason 2012/3/27
		...
		if (SDDevInfo->CtrlID == 3) {
			auto_pll_divisor(DEV_SDMMC3, SET_DIV, 2, 15);
			SDPRINTK("ATSMB Host3 15MHz \n");
		}
		break;
	case SD_Clk_20MHz:
		pSd_Reg->extctl &= ~0x80;  /*  Disable High Speed timing */
		if (SDDevInfo->CtrlID == 0) {
			auto_pll_divisor(DEV_SDMMC0, SET_DIV, 2, 20);//change freq*2 to freq by eason 2012/3/27
		...
		if (SDDevInfo->CtrlID == 3) {
			auto_pll_divisor(DEV_SDMMC3, SET_DIV, 2, 20);
			SDPRINTK("ATSMB Host3 20MHz \n");
		}
		break;
	case SD_Clk_25MHz:
		pSd_Reg->extctl &= ~0x80;  /*  Disable High Speed timing */
		if (SDDevInfo->CtrlID == 0) {
			auto_pll_divisor(DEV_SDMMC0, SET_DIV, 2, 24);//change freq*2 to freq by eason 2012/3/27
		...
		if (SDDevInfo->CtrlID == 3) {
			auto_pll_divisor(DEV_SDMMC3, SET_DIV, 2, 24);
			SDPRINTK("ATSMB Host3 25MHz \n");
		}
		break;
	case SD_Clk_33MHz:
		if (SDDevInfo->SD_Card == SD_TRUE) {
		...
		if (SDDevInfo->CtrlID == 0) {
			auto_pll_divisor(DEV_SDMMC0, SET_DIV, 2, 33);//change freq*2 to freq by eason 2012/3/27
		...
		if (SDDevInfo->CtrlID == 3) {
			auto_pll_divisor(DEV_SDMMC3, SET_DIV, 2, 33);
			SDPRINTK("ATSMB Host3 33MHz \n");
		}
		break;
	case SD_Clk_40MHz:
		if (SDDevInfo->SD_Card == SD_TRUE) {
		...
		if (SDDevInfo->CtrlID == 0) {
			auto_pll_divisor(DEV_SDMMC0, SET_DIV, 2, 40);//change freq*2 to freq by eason 2012/3/27
		...
		if (SDDevInfo->CtrlID == 3) {
			auto_pll_divisor(DEV_SDMMC3, SET_DIV, 2, 40);
			SDPRINTK("ATSMB Host3 40MHz \n");
		}
		break;
    case SD_Clk_44MHz:
		if (SDDevInfo->SD_Card == SD_TRUE) {
		...
		if (SDDevInfo->CtrlID == 0) {
			auto_pll_divisor(DEV_SDMMC0, SET_DIV, 2, 44);//change freq*2 to freq by eason 2012/3/27
		...
		if (SDDevInfo->CtrlID == 3) {
			auto_pll_divisor(DEV_SDMMC3, SET_DIV, 2, 44);
			SDPRINTK("ATSMB Host3 44MHz \n");
		}
		break;
/*	case SD_Clk_50MHz:
		if (SDDevInfo->SD_Card == SD_TRUE) {
		...
		if (SDDevInfo->CtrlID == 0) {
			auto_pll_divisor(DEV_SDMMC0, SET_DIV, 2, 50);//change freq*2 to freq by eason 2012/3/27
	...
	case SD_Clk_400KHz:
		pSd_Reg->extctl &= ~0x80;  /*  Disable High Speed timing */
		if (SDDevInfo->CtrlID == 0) {
			auto_pll_divisor(DEV_SDMMC0, SET_DIV, 1, 400);//change freq*2 to freq by eason 2012/3/27
		...
		if (SDDevInfo->CtrlID == 3) {
			auto_pll_divisor(DEV_SDMMC3, SET_DIV, 1, 400);
			SDPRINTK("ATSMB Host3 400KHz \n");
		}
		break;
	default:
		break;
...
/****************************************************/
{
	...
	if (chip_id == 0x3437 || 
		...
		if (device_num < 0)
			device_num = 0;
	} else if (chip_id == 0x3481){ /* add by eason 2012/3/27*/
		if (device_num > 3)
			device_num = 3;
		if (device_num < 0)
			device_num = 0;
...
//	printf( "debug chip_id = %x, chip_version = %x\n",chip_id,chip_version);
	return chip_version;
}
...
#define GPIO_INPUT_DATA_SD2Data_2		REG8_PTR(GPIO_BASE_ADDR + 0x01A)
#define GPIO_INPUT_DATA_SD2CMD			REG8_PTR(GPIO_BASE_ADDR + 0x018)
#define GPIO_INPUT_DATA_SD2CLK			REG8_PTR(GPIO_BASE_ADDR + 0x018)
#define GPIO_INPUT_DATA_SD2CardDetect	REG8_PTR(GPIO_BASE_ADDR + 0x015)
...
#define GPIO_Enable_SD2Data_2		REG8_PTR(GPIO_BASE_ADDR + 0x05A)
#define GPIO_Enable_SD2CMD			REG8_PTR(GPIO_BASE_ADDR + 0x058)
#define GPIO_Enable_SD2CLK			REG8_PTR(GPIO_BASE_ADDR + 0x058)
#define GPIO_Enable_SD2CardDetect	REG8_PTR(GPIO_BASE_ADDR + 0x055)
...
#define GPIO_OUTPUT_Enable_SD2Data_2		REG8_PTR(GPIO_BASE_ADDR + 0x09A)
...
#define GPIO_OUTPUT_Enable_SD2CardDetect	REG8_PTR(GPIO_BASE_ADDR + 0x095)
...
#define GPIO_OUTPUT_DATA_SD2Data_2		REG8_PTR(GPIO_BASE_ADDR + 0x0DA)
#define GPIO_OUTPUT_DATA_SD2CMD			REG8_PTR(GPIO_BASE_ADDR + 0x0D8)
#define GPIO_OUTPUT_DATA_SD2CLK			REG8_PTR(GPIO_BASE_ADDR + 0x0D8)
#define GPIO_OUTPUT_DATA_SD2CardDetect	REG8_PTR(GPIO_BASE_ADDR + 0x0D5)
...
#define GPIO_PULL_Enable_SD2Data_2		REG8_PTR(GPIO_BASE_ADDR + 0x49A)
#define GPIO_PULL_Enable_SD2CMD			REG8_PTR(GPIO_BASE_ADDR + 0x498)
#define GPIO_PULL_Enable_SD2CLK			REG8_PTR(GPIO_BASE_ADDR + 0x498)
#define GPIO_PULL_Enable_SD2CardDetect	REG8_PTR(GPIO_BASE_ADDR + 0x495)
...
#define GPIO_PULL_Control_SD2Data_2		REG8_PTR(GPIO_BASE_ADDR + 0x4DA)
...
#define GPIO_PULL_Control_SD2CardDetect	REG8_PTR(GPIO_BASE_ADDR + 0x4D5)
...
/*SD3*/
#define GPIO_INPUT_DATA_SD3Data			REG8_PTR(GPIO_BASE_ADDR + 0x00D)
#define GPIO_INPUT_DATA_SD3CMD			REG8_PTR(GPIO_BASE_ADDR + 0x00D)
#define GPIO_INPUT_DATA_SD3CLK			REG8_PTR(GPIO_BASE_ADDR + 0x00D)
#define GPIO_INPUT_DATA_SD3CardDetect	REG8_PTR(GPIO_BASE_ADDR + 0x015)
#define GPIO_INPUT_DATA_SD3WP			REG8_PTR(GPIO_BASE_ADDR + 0x00D)
#define GPIO_INPUT_DATA_SD3PWRSW		REG8_PTR(GPIO_BASE_ADDR + 0x012)

#define GPIO_Enable_SD3Data			REG8_PTR(GPIO_BASE_ADDR + 0x04D)
#define GPIO_Enable_SD3CMD			REG8_PTR(GPIO_BASE_ADDR + 0x04D)
#define GPIO_Enable_SD3CLK			REG8_PTR(GPIO_BASE_ADDR + 0x04D)
#define GPIO_Enable_SD3CardDetect	REG8_PTR(GPIO_BASE_ADDR + 0x055)
#define GPIO_Enable_SD3WP			REG8_PTR(GPIO_BASE_ADDR + 0x04D)
#define GPIO_Enable_SD3PWRSW		REG8_PTR(GPIO_BASE_ADDR + 0x052)

#define GPIO_OUTPUT_Enable_SD3Data			REG8_PTR(GPIO_BASE_ADDR + 0x08D)
#define GPIO_OUTPUT_Enable_SD3CMD			REG8_PTR(GPIO_BASE_ADDR + 0x08D)
#define GPIO_OUTPUT_Enable_SD3CLK			REG8_PTR(GPIO_BASE_ADDR + 0x08D)
#define GPIO_OUTPUT_Enable_SD3CardDetect	REG8_PTR(GPIO_BASE_ADDR + 0x095)
#define GPIO_OUTPUT_Enable_SD3WP			REG8_PTR(GPIO_BASE_ADDR + 0x08D)
#define GPIO_OUTPUT_Enable_SD3PWRSW			REG8_PTR(GPIO_BASE_ADDR + 0x092)

#define GPIO_OUTPUT_DATA_SD3Data		REG8_PTR(GPIO_BASE_ADDR + 0x0CD)
#define GPIO_OUTPUT_DATA_SD3CMD			REG8_PTR(GPIO_BASE_ADDR + 0x0CD)
#define GPIO_OUTPUT_DATA_SD3CLK			REG8_PTR(GPIO_BASE_ADDR + 0x0CD)
#define GPIO_OUTPUT_DATA_SD3CardDetect	REG8_PTR(GPIO_BASE_ADDR + 0x0D5)
#define GPIO_OUTPUT_DATA_SD3WP			REG8_PTR(GPIO_BASE_ADDR + 0x0CD)
#define GPIO_OUTPUT_DATA_SD3PWRSW		REG8_PTR(GPIO_BASE_ADDR + 0x0D2)

#define GPIO_PULL_Enable_SD3Data		REG8_PTR(GPIO_BASE_ADDR + 0x48D)
#define GPIO_PULL_Enable_SD3CMD			REG8_PTR(GPIO_BASE_ADDR + 0x48D)
#define GPIO_PULL_Enable_SD3CLK			REG8_PTR(GPIO_BASE_ADDR + 0x48D)
#define GPIO_PULL_Enable_SD3CardDetect	REG8_PTR(GPIO_BASE_ADDR + 0x495)
#define GPIO_PULL_Enable_SD3WP			REG8_PTR(GPIO_BASE_ADDR + 0x48D)
#define GPIO_PULL_Enable_SD3PWRSW		REG8_PTR(GPIO_BASE_ADDR + 0x492)

#define GPIO_PULL_Control_SD3Data		REG8_PTR(GPIO_BASE_ADDR + 0x4CD)
#define GPIO_PULL_Control_SD3CMD		REG8_PTR(GPIO_BASE_ADDR + 0x4CD)
#define GPIO_PULL_Control_SD3CLK		REG8_PTR(GPIO_BASE_ADDR + 0x4CD)
#define GPIO_PULL_Control_SD3CardDetect	REG8_PTR(GPIO_BASE_ADDR + 0x4D5)
#define GPIO_PULL_Control_SD3WP			REG8_PTR(GPIO_BASE_ADDR + 0x4CD)
#define GPIO_PULL_Control_SD3PWRSW		REG8_PTR(GPIO_BASE_ADDR + 0x4D2)
...
#define	GPIO_SD2_Data			(BIT5 | BIT4 | BIT2)
#define	GPIO_SD2_Data_2			(BIT7)
#define	GPIO_SD2_Command		BIT6
#define	GPIO_SD2_Clock			BIT7
#define GPIO_SD2_CD				BIT6
#define	GPIO_SD2_WriteProtect	BIT3
#define	GPIO_SD2_POWER			BIT1

#define	GPIO_SD3_Data			(BIT7 | BIT6 | BIT5 | BIT4)
#define	GPIO_SD3_Command		BIT2
#define	GPIO_SD3_Clock			BIT1
#define GPIO_SD3_CD				BIT7
...
#define GPIO_SD2_PinShare		(BIT6 | BIT28)
#define GPIO_SD3_PinShare		BIT28


#define SD0_CARD_PWR	BIT1
#define SD1_CARD_PWR	BIT2
#define SD2_CARD_PWR	BIT3
#define SD3_CARD_PWR	BIT3 //check eaon 2012/3/27


	unsigned long SD0_PIN;
	...
	} else if (SDDevInfo->CtrlID == 3) {
		/*set SD3 power pin as GPO pin*/
		*GPIO_Enable_SD3PWRSW |= GPIO_SD3_POWER; /*Set power pin as GPIO pin*/
		*GPIO_OUTPUT_Enable_SD3PWRSW |= GPIO_SD3_POWER; /*Set output pin*/

		/*set internal pull up*/
		*GPIO_PULL_Control_SD3PWRSW |= GPIO_SD3_POWER;
	
		/*set internal pull enable*/
		*GPIO_PULL_Enable_SD3PWRSW |= GPIO_SD3_POWER;
		
		/*disable SD3 power*/
		*GPIO_OUTPUT_DATA_SD3PWRSW |= GPIO_SD3_POWER;
	}
	...
	if (SDDevInfo->CtrlID == 0) {
		...
		*GPIO_PIN_Sharing |= GPIO_SD3_PinShare;
	}
	...
	if (SDDevInfo->CtrlID == 0) {
		...
		*GPIO_Enable_SD2Data_2 |= GPIO_SD2_Data_2;
		...
		*GPIO_Enable_SD3Data |= GPIO_SD3_Data;
		*GPIO_Enable_SD3CMD |= GPIO_SD3_Command;
		*GPIO_Enable_SD3CLK |= GPIO_SD3_Clock;
		*GPIO_Enable_SD3CardDetect |= GPIO_SD3_CD;
		*GPIO_Enable_SD3WP |= GPIO_SD3_WriteProtect;
	}
	...
	if (SDDevInfo->CtrlID == 0) {
		...
		*GPIO_OUTPUT_DATA_SD2Data_2 &= ~GPIO_SD2_Data_2;
		...
		*GPIO_OUTPUT_DATA_SD3Data &= ~GPIO_SD3_Data;
		*GPIO_OUTPUT_DATA_SD3CMD &= ~GPIO_SD3_Command;
		*GPIO_OUTPUT_DATA_SD3CLK &= ~GPIO_SD3_Clock;
        *GPIO_OUTPUT_DATA_SD3CardDetect &= ~GPIO_SD3_CD;
		*GPIO_OUTPUT_DATA_SD3WP &= ~GPIO_SD3_WriteProtect;
	}
	...
	if (SDDevInfo->CtrlID == 0) {
		...
		*GPIO_OUTPUT_Enable_SD2Data_2 |= GPIO_SD2_Data_2;
		...
		*GPIO_OUTPUT_Enable_SD3Data |= GPIO_SD3_Data;
		*GPIO_OUTPUT_Enable_SD3CMD |= GPIO_SD3_Command;
		*GPIO_OUTPUT_Enable_SD3CLK |= GPIO_SD3_Clock;
        *GPIO_OUTPUT_Enable_SD3CardDetect |= GPIO_SD3_CD;
		*GPIO_OUTPUT_Enable_SD3WP |= GPIO_SD3_WriteProtect;
	}
	...
	if (SDDevInfo->CtrlID == 0) {
		...
		*GPIO_PULL_Control_SD2Data_2 |= GPIO_SD2_Data_2;
		...
		*GPIO_PULL_Enable_SD2Data_2 |= GPIO_SD2_Data_2;
		*GPIO_PULL_Enable_SD2WP |= GPIO_SD2_WriteProtect;
	
	} else if (SDDevInfo->CtrlID == 3) {
		/*Disable Clock & CMD Pull enable*/
		*GPIO_PULL_Enable_SD3CLK &= ~GPIO_SD3_Clock;
		*GPIO_PULL_Enable_SD3CMD &= ~GPIO_SD3_Command;
		
		/*Set CD ,WP ,DATA pin pull up*/
                   *GPIO_PULL_Control_SD3CardDetect |= GPIO_SD3_CD;
		*GPIO_PULL_Control_SD3Data |= GPIO_SD3_Data;
		*GPIO_PULL_Control_SD3WP |= GPIO_SD3_WriteProtect;
		
		/*Enable CD ,WP ,DATA  internal pull*/
                   *GPIO_PULL_Enable_SD3CardDetect |= GPIO_SD3_CD;
		*GPIO_PULL_Enable_SD3Data |= GPIO_SD3_Data;
		*GPIO_PULL_Enable_SD3WP |= GPIO_SD3_WriteProtect;
	}
