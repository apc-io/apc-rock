 ...
 * Init USB Device
 */
extern unsigned char port_connect_status ;
			...
			void *data, unsigned short size, int timeout)
{
	...
	/*ehci and uhci port selection*/
	if (usb_flag == usb_ehci_flag)
		submit_control_msg(dev,pipe,data,size,&setup_packet);
	else if  (usb_flag == usb_uhci_flag)
		submit_uhci_control_msg(dev,pipe,data,size,&setup_packet);
	else 
		return -1;
...
int usb_set_maxpacket(struct usb_device *dev)
{
	...
	for (i = 0; i < dev->config.desc.bNumInterfaces; i++)
		/*CharlesTu,20120405,support tool chain arm_1103_le- hang issue.*/
...
int usb_parse_config(struct usb_device *dev, unsigned char *buffer, int cfgno)
{
	...
	int index, ifno, epno, curr_if_num = -1;
	int i;
	unsigned char *ch;
...
struct usb_device * usb_alloc_new_device(void)
{
	...
	if (usb_flag == usb_ehci_flag)
		usb_dev[dev_index].speed=2;  //Charles
