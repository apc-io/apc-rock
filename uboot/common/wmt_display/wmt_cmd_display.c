...
//	govrh_set_tg_enable(VPP_FLAG_DISABLE);
	...
	if (force) {
		...
		s.lineBytes = vfb_var.xres_virtual*(vfb_var.bits_per_pixel>>3);
		mv_initPrimary(&s);
	}
 ...
 */ 
void controlGPIO(void)
{
	printf("[%s] ================ GPIO_Setting ================\n", __func__);
	
#define GPIO_GP0_Enable_Register			0xD8110040
#define GPIO_GP0_Output_Enable_Register		0xD8110080
#define GPIO_GP20_Enable_Register			0xD8110070
#define GPIO_GP20_Output_Enable_Register	0xD81100B0
#define Pin_Sharing_Selection_Register		0xD8110200
	...
	value |= (1 << 5);
	value |= (1 << 6);
    value &= ~(1 << 10);    // Jiun 03/27/2013, GPIO10 disable for PWMOUT2
    value &= ~(1 << 11);    // Jiun 03/27/2013, GPIO11 disable for PWMOUT3
	*addrPtr = value;
	printf("[%s] %08lX = %08lx\n", __func__, addr, *addrPtr);
	...
	*addrPtr = value;
	...
	addr = GPIO_GP20_Enable_Register;
	addrPtr = (long *) addr;
	value = *addrPtr;
	printf("[%s] %08lX = %08lx\n", __func__, addr, value);
	value |= (1 << 4);
	...
	*addrPtr = value;
	...
	addr = GPIO_GP20_Output_Enable_Register;
	addrPtr = (long *) addr;
	value = *addrPtr;
	printf("[%s] %08lX = %08lx\n", __func__, addr, value);
	value &= ~(1 << 4);
	...
	*addrPtr = value;
	...
	addr = Pin_Sharing_Selection_Register;
	addrPtr = (long *) addr;
	value = *addrPtr;
	printf("[%s] %08lX = %08lx\n", __func__, addr, value);
    value |= (1 << 4);      // Jiun 03/27/2013, PWMOUT1/SPI0SS3, 1 = PWMOUT1
    value |= (1 << 5);      // Jiun 03/27/2013, GPIO10&11/PWMOUT2&3, 1 = PWMOUT2&3
	value |= (1 << 8);      // Jiun 03/27/2013, UART/SPI, 1 = SPI
	value &= ~(1 << 14);    // Jiun 03/27/2013, UART0/1, 0=uart1
	*addrPtr = value;
	printf("[%s] %08lX = %08lx\n", __func__, addr, *addrPtr);
...
static int display_show(int format)
{
	 ...
	 * Author: Jason
	 * Date: 2012.09.26
	 * Description: Call function - controlGPIO
	*/ 
	controlGPIO();
