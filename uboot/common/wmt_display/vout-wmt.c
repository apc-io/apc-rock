...
int vout_check_display_info(vout_init_parm_t *init_parm)
{
    ...
    // Jiun 5/24/2012: enable or disable edid reference
    g_vpp.edid_enable = 1;
    if( wmt_getsyspara("wmt.display.edid",buf,&varlen) == 0){
        vpp_parse_param(buf,(unsigned int *)parm,1,0);
        g_vpp.edid_enable = parm[0];
        MSG("edid enable: %d\n", g_vpp.edid_enable);
    }
