...
#if LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,36)
static int  wmt_cec_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg)
#else
static long wmt_cec_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
#endif
{
    int ret = -EINVAL;
