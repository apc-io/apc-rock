...
int ad7393_check_plugin(int hotplug)
{
	...
	vpp_i2c_read(VPP_DVI_I2C_ID,AD7393_ADDR,0x10,&reg,1);
...
void ad7393_set_power_down(int enable)
{
	...
	vpp_i2c_write(VPP_DVI_I2C_ID,AD7393_ADDR,0x0,&reg,1);
}
...
int ad7393_set_mode(unsigned int *option)
{
	...
	wr_buf[0] = 0x2;
	vpp_i2c_write(VPP_DVI_I2C_ID,AD7393_ADDR, 0x17, wr_buf, 1 );	// reset
	...
	vpp_i2c_write(VPP_DVI_I2C_ID,AD7393_ADDR, 0x0, wr_buf, AD7393_REG_NUM);
	return 0;
}	

int ad7393_init(struct vout_s *vo)
{
	unsigned char buf[2];

	buf[0] = 0xff;
	vpp_i2c_read(VPP_DVI_I2C_ID,AD7393_ADDR, 0x0, buf, 1);
...
/*----------------------- vout device plugin --------------------------------------*/
vout_dev_t ad7393_vout_dev_ops = {
	.name = "AD7393",
