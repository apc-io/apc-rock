...
void vppm_set_DVO_select(int govr1)
{
	vppif_reg32_write(VPP_DVO_SEL,govr1); // 1-Dual display use same GOVR timing
}

void vppm_set_NA_mode(int hi_prio)
{
	vppif_reg32_write(VPP_NA_MODE,(hi_prio)?0x2:0x0);
}
...
void vppm_reg_dump(void)
{
	...
	DPRINT("GOVRH2 PVBI(En %d,%d),VBIS(En %d,%d),VBIE(En %d,%d)\n",
		vppif_reg32_read(VPP_GOVRH2_INTEN_PVBI),vppif_reg32_read(VPP_GOVRH2_INTSTS_PVBI),
		vppif_reg32_read(VPP_GOVRH2_INTEN_VBIS),vppif_reg32_read(VPP_GOVRH2_INTSTS_VBIS),
		vppif_reg32_read(VPP_GOVRH2_INTEN_VBIE),vppif_reg32_read(VPP_GOVRH2_INTSTS_VBIE));
	...
	DPRINT("---------- VPP Common ----------\n");
	DPRINT("Dual GOVR %d,NA mode %d\n",(vppif_reg32_read(VPP_DVO_SEL)? 0:1),vppif_reg32_read(VPP_NA_MODE));
}
