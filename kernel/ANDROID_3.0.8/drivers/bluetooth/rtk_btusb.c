...
static struct usb_device_id btusb_table[] = {
	{ USB_DEVICE_AND_INTERFACE_INFO(0x0bda, 0x1724, 0xe0, 0x01, 0x01) },
	{ USB_DEVICE_AND_INTERFACE_INFO(0x0bda, 0x8723, 0xe0, 0x01, 0x01) },
	{ USB_DEVICE_AND_INTERFACE_INFO(0x0bda, 0x0724, 0xe0, 0x01, 0x01) },
	{ }
};
/*******************************/
...
static int btusb_open(struct hci_dev *hdev)
{
	...
	/*******************************/
	err = patch_add(data->intf);
	if (err < 0)
	{
		return -1;
	}

	while(0 == atomic_read(&hdev->promisc))
	{
		msleep(1);
	}
	/*******************************/
...
{
	BT_INFO("Realtek Bluetooth USB driver ver %s", VERSION);
...
typedef struct {
	...
	uint16_t	lmp_sub;
	char*		patch_name;
	const struct firmware *fw_cache;
} patch_info;
...
static int download_patch(dev_data* dev_entry);
...
static patch_info patch_table[] = {
	{ 0x1724, 0x1200, "rtk8723a"},
	{ 0x8723, 0x1200, "rtk8723a"},
	{ 0x0724, 0x1200, "rtk8723a"},
	{ }
};
	...
	void*	unused)
{
	...
	switch (pm_event)
	{
		...
		case PM_POST_RESTORE:
			if (NULL != patch_entry->fw_cache)
			{
				release_firmware(patch_entry->fw_cache);
...
int download_patch(dev_data *dev_entry)
{
	xchange_data		xdata;
	uint8_t				*fw_buf;
	int					ret_val;
...
int get_firmware(xchange_data* xdata)
{
	...
	udev = xdata->dev_entry->udev;
	patch_entry = xdata->dev_entry->patch_entry;
	if (NULL != patch_entry->fw_cache)
	{
		fw = patch_entry->fw_cache;
	}
	else
	{
	...
	xdata->fw_data = kzalloc(fw->size, GFP_KERNEL);
	if (NULL == xdata->fw_data)
	{
		ret_val = -ENOMEM;
		goto alloc_fail;
	}
	memcpy(xdata->fw_data, fw->data, fw->size);
	xdata->fw_len = fw->size;
