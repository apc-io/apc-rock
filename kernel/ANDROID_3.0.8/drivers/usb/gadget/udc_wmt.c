...
#ifdef USE_BULK3_TO_INTERRUPT
	if (((ep->bEndpointAddress & 0x7F) == 3)) {/*Bulk In and Out*/
		if ((unsigned int)retval & 0x00000003) {
			dma_free_coherent(ep->udc->dev, bytes, retval, *dma);
...
#ifdef USE_BULK3_TO_INTERRUPT
		/*pDevReg->InterruptEpControl; // Interrupt endpoint control           - 2C*/
//		pDevReg->Bulk3EpOutEpNum = (addr & 0x7f) << 4;/*Interrupt endpoint reserved byte  - 2D*/
//		pDevReg->Bulk3EpMaxLen = (maxp & 0xFF); /* Interrupt maximum transfer length    - 2E*/
//		pDevReg->Bulk3EpInEpNum = (((addr & 0x7f) << 4) | ((maxp & 0x700) >> 8));
		...
		pDevReg->InterruptReserved = (0x85 & 0x7f) << 4;/*Interrupt endpoint reserved byte  - 2D*/
		pDevReg->InterruptEpMaxLen = (8 & 0xFF); /* Interrupt maximum transfer length    - 2E*/
		pDevReg->InterruptEpEpNum = (((0x85 & 0x7f) << 4) | ((8 & 0x700) >> 8));		
...
#ifdef USE_BULK3_TO_INTERRUPT
	VT8430_INT_EP("ep3in",  USB_DIR_IN  | 3, 28);
#else
