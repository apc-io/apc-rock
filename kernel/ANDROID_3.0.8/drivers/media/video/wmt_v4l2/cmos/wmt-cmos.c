...
int get_cmos_uboot_env(void)
{
	...
	if (wmt_getsyspara("wmt.cmos.dev", varbuf2, &varlen)) {
		...
		sprintf(cmos_env.dev_name ,WMT_DEFAULT_CMOS_DEV);

	} else {
		sscanf(varbuf2, "%s", cmos_env.dev_name);
		printk(" >>> get wmt.cmos.dev %s \n", cmos_env.dev_name);
	}
		

	
		printk("CMOS UBOOT ARG\n");
		printk("get the cmos param  setting wmt.cmos.i2c_gpio : %d:%d:%d:0x%08x:0x%08x:0x%08x:0x%08x:0x%08x:%d:%d:0x%08x:0x%08x:0x%08x:0x%08x:0x%08x\n",
					cmos_env.i2c_gpio_en,				
					cmos_env.i2c_gpio_scl_binum,
					cmos_env.reg_i2c_gpio_scl_gpio_pe_bitnum,
					cmos_env.reg_i2c_gpio_scl_gpio_in,
...
int cmos_cam_s_fmt_cap(struct file *file, void *fh, struct v4l2_format *f)
{
    ...
    init_arg.cmos_v_flip = cmos_env.v_flip;
    init_arg.cmos_h_flip = cmos_env.h_flip;
    init_arg.pix_color_format = f->fmt.pix.pixelformat;
    init_arg.is_rawdata = 0;

    
    ret = cmos_init_ext_device(&init_arg);
...
static int cmos_open(struct file *filp)
{
  ...
  /*--------------------------------------------------------------------------
	...
	wmt_vid_open(VID_MODE_CMOS, &cmos_env);
       ...
       CMOS_REG_SET32( REG_VIDCLK_INV, cmos_get_dev_clk_invert());	//set the clk priority

	 printk("REG_VIDCLK_INV set to %d \n",cmos_get_dev_clk_invert());
	 
	 cmos_isr_delay_count = cmos_get_dev_init_frame_delay_count();
	printk("cmos_isr_delay_count  %d \n",cmos_isr_delay_count);
...
static int cmos_init(void)
{
	...
	dev_i2c_sddr = cmos_get_dev_id(cmos_env.dev_name);
	if (!cmos_env.i2c_gpio_en)
	{
	if (dev_i2c_sddr)
	{
      		ret = wmt_vid_i2c_init(dev_i2c_sddr);
	}else{
