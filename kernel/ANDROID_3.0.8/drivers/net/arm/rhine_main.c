...
static irqreturn_t rhine_intr(int irq, void *dev_instance);
...
static BOOL rhine_alloc_rings(PRHINE_INFO pInfo);
    ...
    case RHINE_INIT_WOL:
        ...
        if (rhine_set_media_mode(&pInfo->hw, &pInfo->hw.sOpts)!=RHINE_LINK_CHANGE) {
            ...
            if (!(status & RHINE_LINK_FAIL)){
                ...
                netif_carrier_on(dev);	//zhf
                netif_wake_queue(dev);
            }
            else
                netif_carrier_off(dev);	//zhf
        }else{
            status = rhine_check_media_mode(&pInfo->hw);
            if(status & RHINE_LINK_FAIL){
                printk("%s: ",pInfo->dev->name);
...
rhine_found1(struct pci_dev *pcid, const struct pci_device_id *ent)
{
	...
	if(wmt_getsyspara(eth_env_name,eth_env_val,&varlen) == 0) {
		...
		if(!(eth & 0x10)) {
            ...
            /* wm3481 support two mode : Mii & EVM */
            /* so disable GPIO to enable MII mode */
	        writeb(0x00, GPIO_CTRL_GP17_MII0RX_BYTE_ADDR);
	        writeb(0x00, GPIO_CTRL_GP19_MII0TX_BYTE_ADDR);
	        writeb(0x00, GPIO_CTRL_GP20_MII0_BYTE_ADDR);
        }
	}else {
	...
	//pInfo->hw.byRevId=__raw_readb(memaddr+0x108);//because  of hard code by PCI driver
	//added by kevin for judge whether update eeprom
	//for(ii=0;ii<6;ii++)
	//mac[ii]=__raw_readb(memaddr+ii);
    ...
    //for (i=0; i<6; i++)
    //    CSR_WRITE_1(&pInfo->hw,rhine_nics*10+i, MAC_REG_PAR+i);
    for (i=0; i<6; i++)
...
static BOOL rhine_init_rings(PRHINE_INFO pInfo) {
    ...
    //void*   vir_pool;
    int     i;
    memset(pInfo->virtual_desc_pool,0, pInfo->hw.sOpts.nRxDescs * sizeof(RX_DESC) +
        pInfo->hw.sOpts.nTxDescs * sizeof(TX_DESC)*pInfo->hw.nTxQueues);

    pInfo->hw.aRDRing=pInfo->virtual_desc_pool;
    ...
    for (i=0;i<pInfo->hw.nTxQueues;i++) {
        ...
        pInfo->hw.apTDRings[i]=pInfo->virtual_desc_pool+
                pInfo->hw.sOpts.nRxDescs*sizeof(RX_DESC)+
                pInfo->hw.sOpts.nTxDescs*sizeof(TX_DESC)*i;
    }
...
static BOOL rhine_init_rd_ring(PRHINE_INFO pInfo) {
    ...
    for (i = 0; i < pInfo->hw.sOpts.nRxDescs; i++, curr+=sizeof(RX_DESC)) {
        ...
        rhine_set_rd_own(pDesc);        //2008/10/14 janshiue patch for small frequent data cause Kernel panic
    }
...
//2008/05/24 janshiue debug
#ifdef MAC_DEBUG
    if(*(skb->data + 0x17) == 0x06) //check TCP protocol
    {
        printk("seq num: %4u : %4X \n", *((DWORD *)(skb->data + 0x26)),*((DWORD *)(skb->data + 0x26)));
    }
#endif
//2008/05/24 janshiue debug
    ...
    if (pInfo->hw.flags & RHINE_FLAGS_VAL_PKT_LEN) {
        if ( (skb->protocol == htons(ETH_P_802_2)) &&
             (skb->len != htons(*(PU16)(skb_mac_header(skb) + 12))) )
...
static int  rhine_open(struct net_device *dev) {
    ...
    i=request_irq(pInfo->pcid->irq, &rhine_intr, IRQF_DISABLED, dev->name, dev);
...
static void rhine_shutdown(PRHINE_INFO pInfo) {
    ...
    udelay(5);
    if(BYTE_REG_BITS_IS_ON(&pInfo->hw, CR0_RXON, MAC_REG_CR0))
    {
        rhine_soft_reset(&pInfo->hw);
        udelay(100);
        if(BYTE_REG_BITS_IS_ON(&pInfo->hw, CR1_SFRST, MAC_REG_CR0))
        {
            // soft reset didn't clear by HW -> force reset
            BYTE_REG_BITS_ON(&pInfo->hw, MISC_CR1_FORSRST, MAC_REG_MISC_CR1);
        }
    }
    //2008/10/15 janshiue-e check RXON is disable, to prevent receiveable and unreceiveable packet are both in ethernet MAC can't STOP issue
    rhine_clearISR(&pInfo->hw);
}
    ...
    if (dev->flags & IFF_PROMISC) {         // Set promiscuous.
        ...
        if (pInfo->hw.flags & RHINE_FLAGS_HAVE_CAM) {
            ...
            netdev_for_each_mc_addr(ha, dev){
                rhine_set_cam(&pInfo->hw,i+offset,ha->addr,RHINE_MULTICAST_CAM);
                mask|=1<<(offset+i);
                i++;
            }
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,11)
                    rhine_suspend(pcid, PMSG_SUSPEND);
#else
