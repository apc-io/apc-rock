			    ...
			    struct mtd_oob_ops *ops)
{
	...
	/*if (col || readlen!=8192)
		printk("enter nand_do_read_ops():------realpage=%d, col=%d, readlen=%d\n", realpage, col, readlen);*/
	//printk("nand_do_read_ops():--readpage=0x%x--ops->mode=%d, mtd->oobavail=%d, mtd->oobsize=%d\n", realpage, ops->mode, mtd->oobavail, mtd->oobsize);
	...
	while (1) {
		...
		/*if (aligned == 0)
			printk("while(1): readlen=%d, bytes=%d, aligned=%d\n",readlen, bytes, aligned);*/
		...
		if (realpage != chip->pagebuf || oob) {
			...
			if (unlikely(oob)) {
				int toread = min(oobreadlen, max_oobsize);
					//printk("oobreadlen=%d, max_oobsize=%d, toread=%d\n",oobreadlen, max_oobsize, toread);
				if (toread) {
			...
			//printk("dannier: read_ops not cmd not sent \n");
			memcpy(buf, chip->buffers->databuf + col, bytes);
			buf += bytes;
		}
...
static int nand_do_read_oob(struct mtd_info *mtd, loff_t from,
			    struct mtd_oob_ops *ops)
{
	int page, realpage, chipnr, sndcmd = 1/*,i*/;
	struct nand_chip *chip = mtd->priv;
	int blkcheck = (1 << (chip->phys_erase_shift - chip->page_shift)) - 1;
	int readlen = ops->ooblen;
	int len;
	...
	//printk("nand_do_read_oob : ops->mode=%d, ops->ooboffs=%d, len=%d, read_len=%d\n",ops->mode, ops->ooboffs, len, readlen);
	if (unlikely(ops->ooboffs >= len)) {
	...
	while (1) {
		...
		len = min(len, readlen);
		buf = nand_transfer_oob(chip, buf, ops, len);
		/*printk(KERN_NOTICE "nand_do_read_oob--------buf1=%x\n", (unsigned int)buf1);
		for (i = 0; i < 48; i += 8)
			printk(KERN_NOTICE "%x %x %x %x %x %x %x %x\n",
			buf1[i], buf1[i+1], buf1[i+2], buf1[i+3], buf1[i+4], buf1[i+5], buf1[i+6], buf1[i+7]);*/
			 ...
			 struct mtd_oob_ops *ops)
{
	...
	if (!ops->datbuf) {
		...
		//printk("enter nand_read_bbt_facmk nand_do_read_bb_oob\n");
	} else {
		//printk("enter nand_read_bbt_facmk nand_do_read_ops \n");
		ret = nand_do_read_ops(mtd, from, ops);
	}
...
//			(unsigned int)instr->addr, (unsigned long long)instr->len);
		    ...
		    struct nand_flash_dev *table)
{
	...
	if (IS_ERR(type)) {
		...
		printk(KERN_WARNING "No NAND device found nand_base!!!\n");
		chip->select_chip(mtd, -1);
		return PTR_ERR(type);
	}
