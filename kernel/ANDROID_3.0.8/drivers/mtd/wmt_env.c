...
struct mtd_info *mtd, *mtd_invalid, *mtd_valid;
static struct mtd_info *mtd_table[2];
...
int read_env_before_mtd_init(int uenv[2], int bootflash, unsigned int offset[2])
{
	...
	//printk(KERN_INFO "read_env_before_mtd_init\n");

	env = uenv[0];
	if ((!mtd_table[0]) && (env != ENV_INVALID) && (bootflash == SPI_FLASH_TYPE || bootflash == NOR_FLASH_TYPE)) {
		#ifdef CONFIG_MTD_WMT_SF
	...
	if ((!mtd_table[0]) && (env != ENV_INVALID)) {
		...
		printk(KERN_WARNING "call IOMAP finish, start read...\n");
		memcpy_fromio((unsigned char *)env_ptr, (unsigned char *)((unsigned int)io_base+offset[i]), ENV_MAX_SIZE);
			if (env_ptr->flags == 1) {
		...
		} else if ((!mtd_table[0]) || (env == ENV_INVALID))
			return 1;
	}
	return 1;
}

int read_env_after_mtd_init(void)
{
	unsigned int i, crc32;
	unsigned char mtd_dev[20];
	size_t retvarlen;

	//printk(KERN_INFO "read_env_after_mtd_init\n");
	env_valid = 0xFF;
	env_invalid = 0xFF;
	for (i = 0; i < 2; i++) {
		sprintf(mtd_dev, "/dev/mtdblock%d", mtd_table[i]->index);
		if (mtd_table[i]->read(
			mtd_table[i],	0, ENV_MAX_SIZE, &retvarlen, (unsigned char *)env_ptr)) {
			printk(KERN_WARNING "## Warning: Read MTD Dev%d fail, retvarlen = %x\n",
			mtd_table[i]->index, retvarlen);
			return 1;
		}
		crc32 = env_crc32(env_ptr->data);
		if (env_ptr->crc == crc32) {
			...
			if (env_ptr->flags == 1) {
				...
				env_invalid = (i == 0)? 1 : 0;
				return 0;
			} else if (env_ptr->flags == 0)
				env_invalid = i;

			if (env_valid != 0xFF && env_invalid != 0xFF) {
				printk(KERN_WARNING "env_valid = 0x%x , env_invalid = 0x%x\n",	env_valid, env_invalid);
				return 0;
			}
		} else
			printk(KERN_WARNING "MTD mtd_table[%d]->index=%d crc32 is not correct\n",	i, mtd_table[i]->index);
	}

	if (env_valid == 0xFF && env_invalid != 0xFF) {
		if (mtd_table[env_invalid]->read(
			mtd_table[env_invalid],	0, ENV_MAX_SIZE, &retvarlen, (unsigned char *)env_ptr)) {
			printk(KERN_WARNING "## Warning: Read MTD Dev%d fail, retvarlen = %x\n",
			mtd_table[env_invalid]->index, retvarlen);
			return 1;
		}
		if (env_invalid == 0) {
			env_valid = 0;
		...
		printk(KERN_WARNING "only one invalid env is found -> take it as valid\n");
		return 0;
	}
	#if 0 /* never happen */
	if (env_valid != 0xFF && env_invalid == 0xFF) {
		if (env_valid == 0)
			...
			env_invalid = 0;
		printk(KERN_WARNING "only one valid env is found\n");
		return 0;
	}
	#endif
	if (env_valid == 0xFF && env_invalid == 0xFF)
		printk(KERN_WARNING "can't find valid and invalid env \n");
	return 1;
}
...
static unsigned int env_init(void)
{
	...
	if (mtd == NULL)
		printk(KERN_WARNING "mtd_for_each_device mtd=0x%x\n", (unsigned int)mtd);*/
	mtd = get_mtd_device_nm(boot_str1);
	//printk(KERN_WARNING "mtd_for_each_device IS_ERR=%d \n", IS_ERR(mtd));
	if (IS_ERR(mtd)) {
		for (i = 0; i < MAX_MTD_DEVICES; i++) {
			...
			(strcmp("w-load-SF", boot_partitions[i].name) || boot_partitions[i].name != NULL)) {
				//printk(KERN_WARNING "scan mtd sf name=%s \n", boot_partitions[i].name);
		...
		mtd = get_mtd_device_nm(boot_str1);
		//printk(KERN_WARNING "scan mtd sf use table mtd=0x%x \n", (unsigned int)mtd);
		if (!IS_ERR(mtd)) {
			mtd_init = 1;
			mtd_table[0] = mtd;
...
next_blk:
		while (offs < (NAND_ENV_BLK_LENGTH * mtd_table[0]->erasesize)) {
			/*printk(KERN_WARNING "## Notice: Read MTD nand Dev%d blk_cnt=%d,", env, blk_cnt);*/
			if (!(mtd_table[0]->block_isbad(mtd_table[0], offs)))
				break;
			else {
				blk_cnt++;
...
static int env_update(void)
{
	...
	unsigned char *io_base = NULL;
	int ret = 0;
//	struct mtd_info *mtd_invalid, *mtd_valid;
	val = *((volatile unsigned int *)(GPIO_BASE_ADDR + 0x100));
	val = (val >> 1) & 0x3;
	if (val == 0)
		bootflash = SPI_FLASH_TYPE;
...
//printk(KERN_WARNING "update 1 mtd_init=%d\n", mtd_init);
	if (mtd_table[env_invalid]) {
		//printk(KERN_WARNING "update 2\n");
...
//printk(KERN_WARNING "update 5\n");
	...
	if (!mtd_table[env_invalid] && bootflash == SPI_FLASH_TYPE && env_invalid != ENV_INVALID) {
		...
		printk(KERN_ERR "no table start sf write..invalid env_valid_offs=0x%x\n", env_valid_offs);
		spi_flash_sector_write(((struct sfreg_t *)SF_BASE_ADDR), io_base,
	...
	{
		if (env_ptr->flags == 0) {
			printk(KERN_WARNING "env_ptr->flags %d set to 1 (when only invlid env found)\n",env_ptr->flags);
			env_ptr->flags = 1;
		}
...
int wmt_getsyspara(char *varname, unsigned char *varval, int *varlen)
{
	...
	if (env_ptr == NULL || (unsigned int)env_ptr != (unsigned int)env_ptr_nand || (!mtd_init)) {
		if (env_ptr == NULL) {
...
//printk(KERN_WARNING "get varname=%s ----------------------------------------------****\n",varname);
	k = -1;
	i = 0;
...
//printk(KERN_WARNING "set varname=%s -----------------------------------------------------\n",varname);
	if (env_ptr == NULL || (unsigned int)env_ptr != (unsigned int)env_ptr_nand || (!mtd_init)) {
		if (env_ptr == NULL) {
			env_ptr = kmalloc(NOR_ENV_MAX_SIZE, GFP_KERNEL);
			if (!env_ptr) /* need copy in RAM */ {
