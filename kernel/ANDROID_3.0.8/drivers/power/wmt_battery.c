...
static int batt_cal_info = 0;   // Jiun 01/15/2013, more battery info
	...
	if (wmt_getsyspara(varname, buf, &varlen) == 0){
		...
		switch( wmt_operation& 0x000F){
			case 0xf:
                ...
                batt_cal_info = 1;
				break;
            // Jiun 01/15/2013, less battery info
            case 0xe:
                batt_operation = 1;
                ADC_USED= 1;
                batt_cal = 1;
...
// Jiun 01/10/2013, display date time
...
static unsigned long wmt_read_capacity(struct power_supply *bat_ps)
{
	...
	if (batt_cal){
        ...
        printk("%d/%02d/%02d %02d:%02d:%02d ADC_val= 0x%04x, %05d\n",
               tm.tm_year+1900, tm.tm_mon+1, tm.tm_mday,
               (tm.tm_hour-(sys_tz.tz_minuteswest/60)), tm.tm_min, tm.tm_sec, ADC_val, ADC_val);
	}
...
static void wmt_bat_update(struct power_supply *bat_ps)
{
    ...
    */
    ...
    // Jiun 01/15/2013, for battery info filter
    //if (batt_cal){
    if (batt_cal_info){
        printk("****************************************************************************  \n");
        printk("bat_low = %d, bat_low_old = %d  \n",bat_low,bat_low_old);
